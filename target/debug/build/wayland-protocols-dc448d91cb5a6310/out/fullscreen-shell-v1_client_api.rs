    ...
    impl super::MessageGroup for Request {
        ...
        fn into_raw(self, sender_id: u32) -> Message {
            match self {
                ...
                Request::PresentSurface { surface, method, output } => Message {
                    sender_id,
                    opcode: 1,
                    args: smallvec![
                ...
                Request::PresentSurfaceForMode { surface, output, framerate } => Message {
                    sender_id,
                    opcode: 2,
                    args: smallvec![
            ...
            F: FnOnce(u32, &mut [wl_argument]) -> T,
        {
            match self {
                ...
                Request::PresentSurface { surface, method, output } => {
                    ...
                    _args_array[1].u = method.to_raw();
                    _args_array[2].o = output
                        .map(|o| o.as_ref().c_ptr() as *mut _)
                        .unwrap_or(::std::ptr::null_mut());
                    f(1, &mut _args_array)
                }
                Request::PresentSurfaceForMode { surface, output, framerate } => {
...
pub mod zwp_fullscreen_shell_mode_feedback_v1 {
    ...
    impl super::MessageGroup for Event {
        const MESSAGES: &'static [super::MessageDesc] = &[
            ...
            super::MessageDesc { name: "mode_failed", since: 1, signature: &[], destructor: false },
            super::MessageDesc {
